...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            ...
            CurrentVacancesFrDatesSensor(
                coordinator=entry.runtime_data.coordinator,
            ),
            DaysRemainingCurrentVacancesFrSensor(
                coordinator=entry.runtime_data.coordinator,
            ),
        ]
    )
...
class CurrentVacancesFrDatesSensor(FrenchHolidayEntity, SensorEntity):
    """french_holidays current dates Sensor class."""
    ...
    def __init__(
        ...
        """Initialize the sensor class."""
        super().__init__(coordinator)
        self.entity_id = f"{Platform.SENSOR}.{DOMAIN}_current_dates_{
            slugify(self.coordinator.config_entry.data['zone'])
        }"
        self.entity_description = SensorEntityDescription(
            key="current_dates",
            name=f"{FRIENDLY_PREFIX} - dates en cours",
            icon="mdi:calendar-range",
        )
    ...
    def _handle_coordinator_update(self) -> None:
        """Update the entity."""
        today = dt.now().date()
        today_event = self.coordinator.get_date_event(today)
        if today_event is not None:
            start_formatted = _format_date_french(today_event.start)
            end_formatted = _format_date_french(today_event.end)
            self._attr_native_value = f"{start_formatted} - {end_formatted}"
            self._attr_extra_state_attributes = get_period_extra_attributes(today_event)
        else:
            self._attr_native_value = None
...
class DaysRemainingCurrentVacancesFrSensor(FrenchHolidayEntity, SensorEntity):
    """french_holidays days remaining in current period Sensor class."""
    ...
    def __init__(
        ...
        """Initialize the sensor class."""
        super().__init__(coordinator)
        self.entity_id = f"{Platform.SENSOR}.{DOMAIN}_days_remaining_current_{
            slugify(self.coordinator.config_entry.data['zone'])
        }"
        self.entity_description = SensorEntityDescription(
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        today_event = self.coordinator.get_date_event(today)
        if today_event is not None:
            days_remaining = (today_event.end - today).days
